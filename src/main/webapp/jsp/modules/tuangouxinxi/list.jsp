...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">团购信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>团购信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">团购信息列表</h3>							
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('shangpinshuliang')">商品数量<i id="shangpinshuliangIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('youhuijia')">优惠价<i id="youhuijiaIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('pintuanrenshu')">拼团人数<i id="pintuanrenshuIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shangpubianhao')">商铺编号<i id="shangpubianhaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shangpumingcheng')">商铺名称<i id="shangpumingchengIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var shangpinshuliangCell  = document.createElement('td');

						 shangpinshuliangCell.innerHTML = item.shangpinshuliang;

			row.appendChild(shangpinshuliangCell);
			var youhuijiaCell  = document.createElement('td');

						 youhuijiaCell.innerHTML = item.youhuijia;

			row.appendChild(youhuijiaCell);
			var pintuanrenshuCell  = document.createElement('td');

						 pintuanrenshuCell.innerHTML = item.pintuanrenshu;

			row.appendChild(pintuanrenshuCell);
			var xiangqingmiaoshuCell  = document.createElement('td');

			
			var shangpubianhaoCell  = document.createElement('td');

						 shangpubianhaoCell.innerHTML = item.shangpubianhao;

			row.appendChild(shangpubianhaoCell);
			var shangpumingchengCell  = document.createElement('td');

						 shangpumingchengCell.innerHTML = item.shangpumingcheng;

			row.appendChild(shangpumingchengCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "加入拼团"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'pintuanshenqing','是','','')");
			if(crossBtnControl('加入拼团') == true){
		...
		function sh(){
			...
			httpJson("tuangouxinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("tuangouxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discusstuangouxinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"tuangouxinxi");
